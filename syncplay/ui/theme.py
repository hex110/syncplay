...
def apply_theme(app: QtWidgets.QApplication) -> None:
    ...
    from syncplay.utils import isWindows, isMacOS

    # 1) Use the Fusion style as a consistent cross-platform base
    app.setStyle(QtWidgets.QStyleFactory.create("Fusion"))

    # 2) Set a modern system font at a consistent size
    if isWindows():
        app.setFont(QtGui.QFont("Segoe UI", 10))
    elif not isMacOS():
        # Linux — prefer common modern sans-serif fonts
        app.setFont(QtGui.QFont("Noto Sans", 10))
    # macOS: leave default (San Francisco is already modern)

    # 3) Apply our custom dark palette
    app.setPalette(_build_dark_palette())

    # 4) Layer the global QSS on top for fine-grained control
    #    Append image-based rules that need the runtime resources path
    checkmark = (resourcespath + 'checkmark.svg').replace('\\', '/')
    image_qss = f"""
    QCheckBox::indicator:checked, QGroupBox::indicator:checked {{
        image: url({checkmark});
    }}
    """
    app.setStyleSheet(DARK_QSS + image_qss)
